...
var GB = {
    ...
    popupWindow: {
        ...
        GoogleMap: {
            tmpl: 'google_map.html',
            width: 538,
            posv: 450,
            title: '구글 지도'
        },
        ColorPicker: {
            tmpl: 'color_picker.html',
            width: 420,
            posv: 200,
            title: '색상 선택'
        },
...
cheditor.prototype = {
    ...
    resetEditArea: function() {
        ...
        var self = this;
        this.addEvent(self.doc.body, "paste", function(event) {
    ...
    getToolbarBgPosition: function(elem) {
        ...
        switch (elem.className) {
            case 'cheditor-tb-bg':
                pos = 3;
                break;
            case 'cheditor-tb-bg-last':
                pos = 6;
                break;
            case 'cheditor-tb-bg-single':
                pos = 9;
                break;
            case 'cheditor-tb-bg30-first':
                pos = 12;
                break;
            case 'cheditor-tb-bg30':
                pos = 15;
                break;
            case 'cheditor-tb-bg30-last':
                pos = 18;
                break;
            case 'cheditor-tb-bg55':
                pos = 21;
                break;
            case 'cheditor-tb-bg40':
                pos = 24;
                break;
            case 'cheditor-tb-bg44':
                pos = 27;
                break;
            case 'cheditor-tb-bgcombo':
                pos = 30;
                break;
            case 'cheditor-tb-bgcombo-last':
                pos = 33;
                break;
            default:
                pos = 0;
    ...
    toolbarUpdate: function() {
        ...
        for (i in toolbar) {
            ...
            switch (cmd) {
                ...
                case 'UnLink':
                    try {
                        this.toolbarDisable(btn, !this.doc.queryCommandEnabled(cmd));
                    } catch (ignore) {}
                    break;
                case 'FormatBlock':
    ...
    showToolbar: function(toolbar, toolbarWrapper) {
        var self = this;
        var i, grpName, btn, btnLen, prevObj, j, attr, btnName, btnObj = null,
            btnNum, spacer, currentColor, fullscreen;
    ...
    createEditorElement: function(container, toolbar) {
        ...
        var self = this,
            i, id, tab, tabId, editArea, done = false;
    ...
    fullScreenMode: function() {
        ...
        if (!self.setFullScreenMode) {
            ...
            self.cheditor.resizeBar.onmousedown = function(event) {
                self.resizeStart(event);
            };
    ...
    doInsertImage: function(images, paragraph, useSpacer) {
        ...
        var i, j = 0,
            attr, image, lastNode = null,
            spacer, len = images.length;
        ...
        for (i in images) {
            ...
            if (paragraph) {
                ...
                if (attr.align !== 'left') {
                    lastNode.style.textAlign = attr.align;
                }
    ...
    xhtmlParse: function(node, lang, needNewLine) {
        var xhtmlText = '',
            innerText, tagName, child, parts, cssText, reMake = [],
            ct, kv, rs, i, j;
        var children = node.childNodes;
        var childrenLength = children.length;
        var doNewLine = needNewLine ? true : false;
        var attr, attrLen, attrValue, attrName, validAttr, attrLang = false,
            attrXmlLang = false,
            attrXmlns = false;
    ...
    setColorTable: function(menu) {
        ...
        var colorPicker = new colorDropper.color(selected, {
            "iconDir": this.config.iconPath
        });
        var mouseOver = function() {
    ...
    doOnKeyDown: function(event) {
        ...
        if (keyCode !== 8 && (keyCode < 33 || keyCode > 40)) {
            return;
        }
        if (this.cheditor.mode === "rich") {
        ...
        if (keyCode && keyCode === 13) {
            ...
            if (GB.browser.msie || GB.browser.edge) {
                ...
                if (storedNode.nodeType === GB.node.element) {
                    ...
                    if (child.nodeType !== GB.node.element) {
                        return;
                    }
            ...
            try {
                ...
                switch (type) {
                    case "Text":
                        ...
                        break;
                    case "Control":
                        selectionType = GB.selection.element;
                        break;
                    case "None":
                        selectionType = GB.selection.none;
                        break;
    ...
    getCssValue: function(elem) {
        ...
        for (i = 0; i < len; i++) {
            k = css[i].split(':');
            style.push({
                "name": this.trimSpace(k[0]),
                "value": this.trimSpace(k[1])
            });
        }
        return style;
    },
    ...
    makeFontCss: function(cmd, opt, elem) {
        switch (cmd) {
            case 'font-size':
                elem.style.fontSize = opt;
                break;
            case 'font-family':
                elem.style.fontFamily = opt;
                break;
            case 'color':
                elem.style.color = opt;
                break;
            case 'background-color':
                elem.style.backgroundColor = opt;
                break;
        }
    },
            ...
            if (cmd === "LineHeight") {
                ...
                if (cmd === "InsertOrderedList" || cmd === "InsertUnOrderedList") {
                    ...
                    var span, nNode, cColor, css, pcss, i, j, len, child, endNode, cur, prev,
                        epoint, compare, collapsed = false,
                        isRemove = [],
                        selection;
                    ...
                    var textCssAttrMatch = function(node, name, value) {
                        ...
                        var style = self.getCssValue(node),
                            elem = null;
                        if (!style) {
                            return null;
                        }
    ...
    modifyImage: function(img) {
        ...
        var getCssFloat = function() {
            if (GB.browser.msie) {
                return img.style.styleFloat;
            }
            return img.style.cssFloat;
        };
        var setCssFloat = function(css) {
            if (GB.browser.msie) {
                img.style.styleFloat = css;
            } else {
                img.style.cssFloat = css;
            }
        };
    ...
    modifyCell: function(ctd) {
        ...
        var ctr = ctb;
        var tm = [],
            i, jr, j, jh, jv, rowIndex = 0,
            realIndex = 0,
            newc, newr, nc, tempr, rows;

        while (ctb !== null && ctb.tagName.toLowerCase() !== "table") {
            ctb = ctb.parentNode;
        }
        while (ctr !== null && ctr.tagName.toLowerCase() !== "tr") {
            ctr = ctr.parentNode;
        }
        ...
        var tblReflash = function() {
            self.editAreaFocus();
            self.doEditorEvent();
        };
        var funcs = {
            'add_cols_after': {
                'icon': 'table_insert_column.png',
                'title': '열 삽입',
                'func': function() {
            ...
            'add_rows_after': {
                'icon': 'table_insert_row.png',
                'title': '행 삽입',
                'func': function() {
                    insertRow(ctr.rowIndex);
                    tblReflash();
                }
            },
            'remove_cols': {
                'icon': 'table_delete_column.png',
                'title': '열 삭제',
                'func': function() {
    ...
    doEditorEvent: function() {
        ...
        if (self.config.showTagPath) {
            ...
            var alinkClick = function() {
                self.$('removeSelected').style.display = 'inline';
...
GB.prettify = new function() {
    ...
    this.LANGUAGES.xml = {
        ...
        modes: [{
            ...
            PI, DOCTYPE, COMMENT, TAG, this.inherit(TITLE, {
                relevance: 1.75
            }), TAG_INTERNAL, ATTR, VALUE_CONTAINER_QUOT, VALUE_CONTAINER_APOS
        ]
    };
    ...
    this.LANGUAGES.html = {
        ...
        modes: [{
            ...
            COMMENT, PI, DOCTYPE, this.inherit(TAG), this.inherit(TITLE, {
                lexems: [this.IDENT_RE],
                keywords: HTML_TAGS
            }),
    ...
    this.LANGUAGES.javascript = {
        defaultMode: {
            lexems: [this.UNDERSCORE_IDENT_RE],
        ...
        modes: [this.C_LINE_COMMENT_MODE, this.C_BLOCK_COMMENT_MODE, this.C_NUMBER_MODE, this.APOS_STRING_MODE, this.QUOTE_STRING_MODE, this.BACKSLASH_ESCAPE,
            {
                ...
                begin: '(' + this.RE_STARTERS_RE + '|case|return|throw)\\s*',
                end: this.IMMEDIATE_RE,
                noMarkup: true,
                lexems: [this.IDENT_RE],
                keywords: {
    ...
    this.LANGUAGES.css = {
        ...
        modes: [{
            ...
            this.APOS_STRING_MODE, this.QUOTE_STRING_MODE, this.BACKSLASH_ESCAPE
        ]
    };
}();
...
var colorDropper = {
    ...
    color: function(target, prop) {
        ...
        var abortBlur = false;
        var valueElement = colorDropper.fetchElement(this.valueElement),
        ...
        function drawPicker() {
            ...
            if ('ontouchstart' in window) {
                p.padM.addEventListener('touchstart', function(e) {
                    touchOffset = {
                        'X': getOffsetParent(e.target).Left,
                        'Y': getOffsetParent(e.target).Top
                    };
                    this.onmousedown({
                        'offsetX': e.touches[0].pageX - touchOffset.X,
                        'offsetY': e.touches[0].pageY - touchOffset.Y
